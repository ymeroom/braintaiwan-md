...
> **For agentic workers:** 由主 agent（Opus）在自己的 loop 裡逐 Task 執行，**不要用 subagent 逐篇跑**（2026-07-21 實測：subagent 版燒完整個 5 小時 usage window 只完成 1／7 篇）。Steps 用 checkbox (`- [ ]`) 追蹤。
...
- **第 3 頁 graphical abstract 的 `"> or ="`／`"< or ="` 是 BSR 原圖所印，不是 OCR 瑕疵**（已對 PDF 第 3 頁目視核校）。該區塊**可正常作為 `sourceQuote` 來源**，字面即與 `_source.md` 相符；文章正文引用時把符號正規化為 `≥`／`≤`。2016 ACR/EULAR 五個計分項只出現在此區塊，本文他處沒有——不得因此略過不寫。
- 撰寫任何說明文字時，勿寫出字面的頁碼標記語法 —— 驗證用的正則會把它一起數進去。
...
- [x] **Step 2: 確認 cidp 頁面的 enhancer 內容確實存在**
...
- [x] **Step 3: 在 `_source.md` 開頭寫入來源與引擎記錄**
...
- [x] **Step 1: 建立 `series.json`（7 篇條目全填）**
...
- [x] **Step 2: 註冊 `sjd` 到 footer enhancer**
...
- [x] **Step 4: 建立 `_gate.js` 並寫入 sjd01 的 claim ledger**
...
- [x] **Step 4: 回填 series.json 第 2 筆卡片**
...
- [x] **Step 4: 回填 series.json 第 3 筆卡片**
...
- [x] **Step 4: 回填 series.json 第 4 筆卡片**
...
- [x] **Step 4: 回填 series.json 第 5 筆卡片**
...
- [x] **Step 4: 回填 series.json 第 6 筆卡片**
...
- [x] **Step 4: 回填 series.json 第 7 筆卡片**
...
- [x] **Step 1: 確認 series.json 完整、無佔位，並清除冒煙用副本**
