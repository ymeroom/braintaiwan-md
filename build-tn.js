// 將 tn-articles 的 5 篇 .md 轉成站內風格 HTML 頁面
const fs = require('fs');
const path = require('path');

const SRC = 'D:/claudecode/tn-articles';
const OUT = __dirname;
const SERIES_TAG = '三叉神經痛：從臨床困境到分子新知';

const articles = [
  { md: '01-clinical-challenges.md', out: 'tn01.html', nav: '① 臨床困境與分類' },
  { md: '02-pain-circuits.md',       out: 'tn02.html', nav: '② 痛覺傳遞的 3D 迴路' },
  { md: '03-ion-channels.md',        out: 'tn03.html', nav: '③ 離子通道微觀戰場' },
  { md: '04-neuroimmune.md',         out: 'tn04.html', nav: '④ 神經免疫與表觀遺傳' },
  { md: '05-pharmacotherapy.md',     out: 'tn05.html', nav: '⑤ 藥物進展與未來' },
];
...
function inline(s){
  ...
  s = s.replace(/\$([^\$]+)\$/g, (m,c)=>{ codes.push(c); return `\x00${codes.length-1}\x00`; }); // Handle basic math/symbols
  s = esc(s);
